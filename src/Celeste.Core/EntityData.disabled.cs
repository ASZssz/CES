...
public class EntityData
{
	...
	public Vector2[] NodesOffset(Vector2 offset)
	{
		...
		Vector2[] array = new Vector2[Nodes.Length];
		for (int i = 0; i < Nodes.Length; i++)
		{
			array[i] = Nodes[i] + offset;
		}
		return array;
	}
	...
	public Vector2[] NodesWithPosition(Vector2 offset)
	{
		...
		Vector2[] array = new Vector2[Nodes.Length + 1];
		array[0] = Position + offset;
		for (int i = 0; i < Nodes.Length; i++)
		{
			array[i + 1] = Nodes[i] + offset;
		}
		return array;
	}
	...
	{
		// Calc.HexToColor não disponível - retorna padrão
		return defaultValue;
	}
}
