	...
	public void Simulate(float duration)
	{
		...
		for (int i = 0; (float)i < num; i++)
		{
			for (int j = 0; j < Amount; j++)
			{
				...
				Vector2 position = base.Entity.Position + Position + Calc.Random.Range(-Range, Range);
				particle = ((!Direction.HasValue) ? Type.Create(ref particle, position) : Type.Create(ref particle, position, Direction.Value));
				particle.Track = Track;
				float duration2 = duration - Interval * (float)i;
