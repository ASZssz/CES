...
public class WaterFall : Entity
{
	...
	public override void Update()
	{
		...
		{
			Vector2 position2 = new Vector2(base.X + 4f, base.Y + height + 2f);
	...
	public override void Render()
	{
		...
		for (int i = 0; i < 6; i++)
		{
			Draw.Rect(base.X + (float)i + 1f, base.Y, 1f, num - topSurface.GetSurfaceHeight(new Vector2(base.X + 1f + (float)i, water.Y)), Water.FillColor);
		}
		Draw.Rect(base.X - 1f, base.Y, 2f, num - topSurface.GetSurfaceHeight(new Vector2(base.X, water.Y)), Water.SurfaceColor);
		Draw.Rect(base.X + 7f, base.Y, 2f, num - topSurface.GetSurfaceHeight(new Vector2(base.X + 8f, water.Y)), Water.SurfaceColor);
	}
}
