...
public class NorthernLights : Backdrop
{
	private class Strand
	{
		...
		public void Reset(float startPercent)
		{
			...
			Alpha = 0f;
			Nodes.Clear();
			Vector2 position = new Vector2(Calc.Random.Range(-40, 60), Calc.Random.Range(40, 90));
	...
	public override void BeforeRender(Scene scene)
	{
		...
		for (int j = 0; j < particles.Length; j++)
		{
			...
			{
				X = mod(particles[j].Position.X - camera.X * 0.2f, 320f),
				Y = mod(particles[j].Position.Y - camera.Y * 0.2f, 180f)
