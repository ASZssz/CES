...
public class PreviewDialog : Scene
{
	...
	public override void Update()
	{
		...
		if (current != null)
		{
			...
			foreach (object element in elements)
			{
				...
				{
					num += (float)(language.FontSize.LineHeight + 50);
				}
			}
			...
			else if (MInput.Keyboard.Pressed(Keys.Space))
			{
				string item = current;
				ClearTextboxes();
				int num2 = list.IndexOf(item) + 1;
				if (num2 < list.Count)
				{
					SetCurrent(list[num2]);
				}
			}
		}
		else
		...
		if (MInput.Keyboard.Pressed(Keys.F2))
		{
			...
			Engine.Scene = new PreviewDialog(this.language, listScroll.Y, textboxScroll.Y, current);
		}
		if (MInput.Keyboard.Pressed(Keys.F1) && this.language != null)
		{
			Celeste.ReloadDialog();
			Engine.Scene = new PreviewDialog(Dialog.Languages[this.language.Id], listScroll.Y, textboxScroll.Y, current);
		}
	}
	...
	private void SetCurrent(string id)
	{
		...
		while (true)
		{
			...
			if (textbox != null)
			{
				for (int i = textbox.Start + 1; i <= textbox2.Start && i < textbox.Nodes.Count; i++)
				{
					if (textbox.Nodes[i] is FancyText.Trigger trigger)
					{
			...
			textbox2.RenderOffset = textboxScroll + Vector2.UnitY * (1 + num * 300);
			textbox = textbox2;
			num++;
		}
	}
	...
	private void SetLanguage(Language lan)
	{
		...
		{
			if (!flag && item.Key.StartsWith("CH0", StringComparison.OrdinalIgnoreCase))
			{
				flag = true;
			}
			if (flag && !item.Key.StartsWith("poem_", StringComparison.OrdinalIgnoreCase) && !item.Key.StartsWith("journal_", StringComparison.OrdinalIgnoreCase))
			{
				list.Add(item.Key);
			}
		}
	...
	private void RenderContent()
	{
		...
		if (current != null)
		{
			...
			foreach (object element in elements)
			{
				if (element is Textbox textbox)
				{
					...
					{
						textbox.Render();
						language.Font.DrawOutline(language.FontFaceSize, "#" + num, textbox.RenderOffset + new Vector2(32f, 64f), Vector2.Zero, Vector2.One * 0.5f, Color.White, 2f, Color.Black);
						num++;
						num2 += 300;
					}
				}
				...
				{
					language.Font.DrawOutline(language.FontFaceSize, element.ToString(), textboxScroll + new Vector2(128f, num2 + 50 + language.FontSize.LineHeight), new Vector2(0f, 0.5f), Vector2.One * 0.5f, Color.White, 2f, Color.Black);
		...
		else if (language != null)
		{
			...
			foreach (string item in list)
			{
				...
				{
					language.Font.Draw(language.FontFaceSize, item, listScroll + new Vector2(0f, (float)num3 * ActiveFont.LineHeight * 0.6f), Vector2.Zero, Vector2.One * 0.6f, MouseOverOption(num3) ? Color.White : Color.Gray);
				}
				num3++;
			}
		}
		else
		{
			...
			foreach (KeyValuePair<string, Language> language in Dialog.Languages)
			{
				ActiveFont.Draw(language.Value.Id, listScroll + new Vector2(0f, (float)num4 * ActiveFont.LineHeight * 0.6f), Vector2.Zero, Vector2.One * 0.6f, MouseOverOption(num4) ? Color.White : Color.Gray);
				num4++;
			}
		}
