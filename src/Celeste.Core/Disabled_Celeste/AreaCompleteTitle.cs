...
public class AreaCompleteTitle : Entity
{
	...
	public AreaCompleteTitle(Vector2 origin, string text, float scale, bool rainbow = false)
	{
		...
		Vector2 vector2 = origin + Vector2.UnitY * vector.Y * 0.5f + Vector2.UnitX * vector.X * -0.5f;
		for (int i = 0; i < text.Length; i++)
		{
			Vector2 vector3 = ActiveFont.Measure(text[i].ToString()) * scale;
			if (text[i] != ' ')
			{
				Letter letter = new Letter(i, text[i].ToString(), vector2 + Vector2.UnitX * vector3.X * 0.5f);
				if (rainbow)
				{
			...
			vector2 += Vector2.UnitX * vector3.X;
		}
