...
public class Pathfinder
{
	...
	public bool Find(ref List<Vector2> path, Vector2 from, Vector2 to, bool fewerTurns = true, bool logging = false)
	{
		...
		for (int i = 0; i < num3; i++)
		{
			...
			{
				map[i, j].Solid = level.SolidsData[i + levelSolidOffset.X, j + levelSolidOffset.Y] != '0';
				map[i, j].Cost = int.MaxValue;
				map[i, j].Parent = null;
			}
		}
		...
		Point point6 = point2;
		int num12 = 0;
		while (point6 != point && num12++ < 1000)
		{
			path.Add(new Vector2((float)point6.X + 0.5f, (float)point6.Y + 0.5f) * 8f + level.LevelOffset);
			point6 = map[point6.X, point6.Y].Parent.Value;
		}
		if (num12 >= 1000)
		{
