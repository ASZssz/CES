...
public class RisingLava : Entity
{
	...
	public override void Update()
	{
		...
		else
		{
			...
			if (base.Top > num2 + 96f)
			{
				base.Top = num2 + 96f;
			}
			num = ((!(base.Top > num2)) ? Calc.ClampedMap(num2 - base.Top, 0f, 32f, 1f, 0.5f) : Calc.ClampedMap(base.Top - num2, 0f, 96f, 1f, 2f));
			if (delay <= 0f)
			{
				loopSfx.Param("rising", 1f);
				base.Y += -30f * num * Engine.DeltaTime;
			}
		}
