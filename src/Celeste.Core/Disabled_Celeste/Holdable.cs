...
public class Holdable : Component
{
	...
	public void Release(Vector2 force)
	{
		if (base.Entity.CollideCheck<Solid>())
		{
			if (force.X != 0f)
			{
				...
				while (!flag && num2++ < 10)
				{
					if (!base.Entity.CollideCheck<Solid>(base.Entity.Position + num * num2 * Vector2.UnitX))
					{
						flag = true;
					}
				}
	...
	{
		foreach (HoldableCollider component in base.Scene.Tracker.GetComponents<HoldableCollider>())
		{
