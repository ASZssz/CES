...
public class Spritesheet<T> : Image
{
	...
	public void SetFrames(MTexture texture, int frameWidth, int frameHeight, int frameSep = 0)
	{
		...
		int i = 0;
		int num = 0;
		while (num <= texture.Height - frameHeight)
		{
			for (; i <= texture.Width - frameWidth; i += frameWidth + frameSep)
			{
				list.Add(texture.GetSubtexture(i, num, frameWidth, frameHeight));
			}
			num += frameHeight + frameSep;
			i = 0;
		}
		Frames = list.ToArray();
	}
