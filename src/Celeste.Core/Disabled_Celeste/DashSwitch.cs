...
public class DashSwitch : Solid
{
	...
	public static DashSwitch Create(EntityData data, Vector2 offset, EntityID id)
	{
		...
		if (data.Name.Equals("dashSwitchH"))
		{
			...
			{
				return new DashSwitch(position, Sides.Left, flag, flag2, id, spriteName);
			}
			return new DashSwitch(position, Sides.Right, flag, flag2, id, spriteName);
		}
		if (data.Bool("ceiling"))
		{
			return new DashSwitch(position, Sides.Up, flag, flag2, id, spriteName);
		}
		return new DashSwitch(position, Sides.Down, flag, flag2, id, spriteName);
	}
