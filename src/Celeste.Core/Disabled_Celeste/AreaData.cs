...
public class AreaData
{
	...
	public static void Load()
	{
		...
		for (int num2 = 0; num2 < Areas.Count; num2++)
		{
			Areas[num2].ID = num2;
			Areas[num2].Mode[0].MapData = new MapData(new AreaKey(num2));
			if (Areas[num2].Interlude)
			{
				continue;
			}
			for (int num3 = 1; num3 < num; num3++)
			{
				if (Areas[num2].HasMode((AreaMode)num3))
				{
					Areas[num2].Mode[num3].MapData = new MapData(new AreaKey(num2, (AreaMode)num3));
				}
			}
		}
		ReloadMountainViews();
	...
	{
		foreach (XmlElement item in Calc.LoadXML(Path.Combine(Engine.ContentDirectory, "Overworld", "AreaViews.xml"))["Views"])
		{
			int num = item.AttrInt("id");
			if (num >= 0 && num < Areas.Count)
			{
				Vector3 pos = item["Idle"].AttrVector3("position");
				Vector3 target = item["Idle"].AttrVector3("target");
				Areas[num].MountainIdle = new MountainCamera(pos, target);
				pos = item["Select"].AttrVector3("position");
				target = item["Select"].AttrVector3("target");
				Areas[num].MountainSelect = new MountainCamera(pos, target);
				pos = item["Zoom"].AttrVector3("position");
				target = item["Zoom"].AttrVector3("target");
				Areas[num].MountainZoom = new MountainCamera(pos, target);
				if (item["Cursor"] != null)
				{
					Areas[num].MountainCursor = item["Cursor"].AttrVector3("position");
				}
				Areas[num].MountainState = item.AttrInt("state", 0);
			}
		}
	}
	...
	public static CheckpointData GetCheckpoint(AreaKey area, string level)
	{
		...
		{
			CheckpointData[] array = checkpoints;
