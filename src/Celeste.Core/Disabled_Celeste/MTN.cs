...
public static class MTN
{
	...
	public static void LoadData()
	{
		if (!DataLoaded)
		{
			Stopwatch stopwatch = Stopwatch.StartNew();
			string text = ".obj";
			MountainTerrain = ObjModel.Create(Path.Combine(Engine.ContentDirectory, "Overworld", "mountain" + text));
			MountainBuildings = ObjModel.Create(Path.Combine(Engine.ContentDirectory, "Overworld", "buildings" + text));
			MountainCoreWall = ObjModel.Create(Path.Combine(Engine.ContentDirectory, "Overworld", "mountain_wall" + text));
			MountainMoon = ObjModel.Create(Path.Combine(Engine.ContentDirectory, "Overworld", "moon" + text));
			MountainBird = ObjModel.Create(Path.Combine(Engine.ContentDirectory, "Overworld", "bird" + text));
