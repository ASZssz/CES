		...
		public override void Render(Vector2 position, float alignment = 0.5f, float scale = 1f)
		{
			...
			for (int i = 0; i < Credits.Length; i++)
			{
				if (linkedImages[i] != null)
				{
					...
					mTexture.DrawJustified(position.Floor() + new Vector2(0f, -2f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.Floor() + new Vector2(0f, 2f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.Floor() + new Vector2(-2f, 0f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.Floor() + new Vector2(2f, 0f), new Vector2(alignment, 0f), BorderColor, 1.15f * scale, 0f);
					mTexture.DrawJustified(position.Floor(), new Vector2(alignment, 0f), CreditsColor, 1.15f * scale, 0f);
