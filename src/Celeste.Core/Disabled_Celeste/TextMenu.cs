...
public class TextMenu : Entity
{
	...
	public class Setting : Item
	{
		...
		public void Set(List<Keys> keys)
		{
			Values.Clear();
			int i = 0;
			for (int num = Math.Min(Input.MaxBindings, keys.Count); i < num; i++)
			{
				if (keys[i] == Keys.None)
				{
					continue;
		...
		public void Set(List<Buttons> buttons)
		{
			Values.Clear();
			int i = 0;
			for (int num = Math.Min(Input.MaxBindings, buttons.Count); i < num; i++)
			{
		...
		public override float RightWidth()
		{
			...
			foreach (object value in Values)
			{
				...
				{
					num += (float)(value as MTexture).Width;
				}
	...
	[CompilerGenerated]
	private sealed class _003CCloseAndRunRoutine_003Ed__54 : IEnumerator<object>, IDisposable, IEnumerator
	{
		...
		public TextMenu _003C_003E4__this;
		...
		[DebuggerHidden]
		public _003CCloseAndRunRoutine_003Ed__54(int _003C_003E1__state)
		{
		...
		private bool MoveNext()
		{
			...
			TextMenu textMenu = _003C_003E4__this;
			switch (num)
			{
			default:
				return false;
	...
	{
		float num = (Height = 0f);
		float leftColumnWidth = (RightColumnWidth = num);
	...
	[IteratorStateMachine(typeof(_003CCloseAndRunRoutine_003Ed__54))]
	...
	{
		RecalculateSize();
		Vector2 vector = Position - Justify * new Vector2(Width, Height);
		...
		foreach (Item item in items)
		{
			if (item.Visible)
			{
				...
				if (!item.AboveAll)
				{
					item.Render(vector2 + new Vector2(0f, num * 0.5f + item.SelectWiggler.Value * 8f), Focused && Current == item);
				}
				else
				{
					flag = true;
				}
				vector2.Y += num + ItemSpacing;
			}
		}
		if (!flag)
		{
		...
		foreach (Item item2 in items)
		{
			if (item2.Visible)
			{
				...
				if (item2.AboveAll)
				{
					item2.Render(vector2 + new Vector2(0f, num2 * 0.5f + item2.SelectWiggler.Value * 8f), Focused && Current == item2);
				}
				vector2.Y += num2 + ItemSpacing;
			}
		}
	}
}
