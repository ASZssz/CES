		...
		: base(journal)
	{
		...
		foreach (AreaStats area in SaveData.Instance.Areas)
		{
			...
			Row row = table.AddRow().Add(new TextCell(Dialog.Clean(areaData.Name), new Vector2(1f, 0.5f), 0.6f, TextColor)).Add(null)
				.Add(iconsCell = new IconsCell(CompletionIcon(area)));
			if (areaData.CanFullClear)
			{
				row.Add(new IconsCell(area.Cassette ? "cassette" : "dot"));
			...
			if (areaData.IsFinal)
			{
				row.Add(new TextCell(Dialog.Deaths(area.Modes[0].Deaths), TextJustify, 0.5f, TextColor)
				{
			...
			else
			{
				for (int k = 0; k < SaveData.Instance.UnlockedModes; k++)
				{
					...
					{
						row.Add(new TextCell(Dialog.Deaths(area.Modes[k].Deaths), TextJustify, 0.5f, TextColor));
					}
					else
					{
			...
			{
				row.Add(new TextCell(Dialog.Time(area.TotalTimePlayed), TextJustify, 0.5f, TextColor));
			}
			else
			{
