...
public class FancyText
{
	...
	public class Text
	{
		...
		public void Draw(Vector2 position, Vector2 justify, Vector2 scale, float alpha, int start = 0, int end = int.MaxValue)
		{
			...
			for (int j = start; j < num && !(Nodes[j] is NewPage); j++)
			{
				...
				if (Nodes[j] is Char)
				{
					Char obj = Nodes[j] as Char;
					obj.Draw(Font, BaseSize, position, scale, alpha);
					num3 = Math.Max(num3, obj.Scale);
				}
			}
		}
		...
		public void DrawJustifyPerLine(Vector2 position, Vector2 justify, Vector2 scale, float alpha, int start = 0, int end = int.MaxValue)
		{
			...
			for (int j = start; j < num && !(Nodes[j] is NewPage); j++)
			{
				...
				{
					Char obj = Nodes[j] as Char;
					Vector2 vector = -justify * new Vector2(obj.LineWidth, num3 * (float)pixelFontSize.LineHeight) * scale;
					obj.Draw(Font, BaseSize, position + vector, scale, alpha);
					num2 = Math.Max(num2, obj.Scale);
				}
			}
		}
	}
	...
	private void CalcLineWidth()
	{
		...
		while (num >= 0 && obj == null)
		{
			...
			{
				obj = group.Nodes[num] as Char;
			}
			else if (group.Nodes[num] is NewLine || group.Nodes[num] is NewPage)
			{
				return;
			}
			num--;
		...
		float lineWidth = (obj.LineWidth = obj.Position + (float)size.Get(obj.Character).XAdvance * obj.Scale);
		while (num >= 0 && !(group.Nodes[num] is NewLine) && !(group.Nodes[num] is NewPage))
		{
			if (group.Nodes[num] is Char)
			{
	...
	private void AddWord(string word)
	{
		...
		for (int i = 0; i < word.Length; i++)
		{
			...
			group.Nodes.Add(new Char
			{
				...
				Line = currentLine,
				Page = currentPage,
				Delay = (currentImpact ? 0.0034999999f : (currentDelay + num2)),
				Color = currentColor,
				Scale = currentScale,
