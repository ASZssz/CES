...
public class MoonParticle3D : Entity
{
	public class Particle : Billboard
	{
		...
		public override void Update()
		{
			...
			Vector3 position = new Vector3((float)Math.Cos(Rotation) * Distance, (float)Math.Sin(Rotation * 3f) * 0.25f + YOff, (float)Math.Sin(Rotation) * Distance);
	...
	public MoonParticle3D(MountainModel model, Vector3 center)
	{
		...
		for (int i = 0; i < 20; i++)
		{
			Add(new Particle(OVR.Atlas["star"], Calc.Random.Choose(choices), center, 1f, matrix));
		}
		for (int j = 0; j < 30; j++)
		{
			Add(new Particle(OVR.Atlas["snow"], Calc.Random.Choose(choices), center, 0.3f, matrix));
		}
		Matrix matrix2 = Matrix.CreateRotationZ(0.8f) * Matrix.CreateRotationX(0.4f);
		Color[] choices2 = new Color[2]
		{
		...
		for (int k = 0; k < 20; k++)
		{
			Add(new Particle(OVR.Atlas["star"], Calc.Random.Choose(choices2), center, 1f, matrix2));
		}
		for (int l = 0; l < 30; l++)
		{
			Add(new Particle(OVR.Atlas["snow"], Calc.Random.Choose(choices2), center, 0.3f, matrix2));
		}
	}
