...
public class AutoSavingNotice : Renderer
{
	...
	public override void Update(Scene scene)
	{
		...
		bool flag = ForceClose || (!Display && timer >= 1f);
		ease = Calc.Approach(ease, (!flag) ? 1 : 0, Engine.DeltaTime);
	...
	public override void Render(Scene scene)
	{
		...
		ActiveFont.Draw(Dialog.Clean("autosaving_title_PC"), new Vector2(960f, 480f - 30f * num), new Vector2(0.5f, 1f), Vector2.One, color);
		if (icon.Visible)
		{
			icon.RenderPosition = new Vector2(1920f, 1080f) / 2f;
			icon.Render();
		}
		ActiveFont.Draw(Dialog.Clean("autosaving_desc_PC"), new Vector2(960f, 600f + 30f * num), new Vector2(0.5f, 0f), Vector2.One, color);
