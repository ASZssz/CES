...
public static class Draw
{
	...
	public static void Circle(Vector2 position, float radius, Color color, float thickness, int resolution)
	{
		...
		for (int i = 1; i <= resolution; i++)
		{
			...
			Line(position - vector, position - vector3, color, thickness);
			Line(position + vector2, position + vector4, color, thickness);
			Line(position - vector2, position - vector4, color, thickness);
			vector = vector3;
			vector2 = vector4;
		}
	...
	public static void OutlineTextCentered(SpriteFont font, string text, Vector2 position, Color color, float scale)
	{
		...
		for (int i = -1; i < 2; i++)
		{
			for (int j = -1; j < 2; j++)
			{
				if (i != 0 || j != 0)
				{
					SpriteBatch.DrawString(font, text, position + new Vector2(i, j), Color.Black, 0f, origin, scale, SpriteEffects.None, 0f);
				}
			}
		}
		SpriteBatch.DrawString(font, text, position, color, 0f, origin, scale, SpriteEffects.None, 0f);
	...
	public static void OutlineTextCentered(SpriteFont font, string text, Vector2 position, Color color, Color outlineColor)
	{
		...
		for (int i = -1; i < 2; i++)
		{
			for (int j = -1; j < 2; j++)
			{
				if (i != 0 || j != 0)
				{
					SpriteBatch.DrawString(font, text, position + new Vector2(i, j), outlineColor, 0f, origin, 1f, SpriteEffects.None, 0f);
				}
			}
		}
		SpriteBatch.DrawString(font, text, position, color, 0f, origin, 1f, SpriteEffects.None, 0f);
	...
	public static void OutlineTextCentered(SpriteFont font, string text, Vector2 position, Color color, Color outlineColor, float scale)
	{
		...
		for (int i = -1; i < 2; i++)
		{
			for (int j = -1; j < 2; j++)
			{
				if (i != 0 || j != 0)
				{
					SpriteBatch.DrawString(font, text, position + new Vector2(i, j), outlineColor, 0f, origin, scale, SpriteEffects.None, 0f);
				}
			}
		}
		SpriteBatch.DrawString(font, text, position, color, 0f, origin, scale, SpriteEffects.None, 0f);
	...
	public static void SineTextureH(MTexture tex, Vector2 position, Vector2 origin, Vector2 scale, float rotation, Color color, SpriteEffects effects, float sineCounter, float amplitude = 2f, int sliceSize = 2, float sliceAdd = (float)Math.PI / 4f)
	{
		...
		{
			Vector2 vector = new Vector2(sliceSize * num, (float)Math.Round(Math.Sin(sineCounter + sliceAdd * (float)num) * (double)amplitude));
			SpriteBatch.Draw(tex.Texture.Texture, position, clipRect, color, rotation, origin - vector, scale, effects, 0f);
			num++;
	...
	public static void SineTextureV(MTexture tex, Vector2 position, Vector2 origin, Vector2 scale, float rotation, Color color, SpriteEffects effects, float sineCounter, float amplitude = 2f, int sliceSize = 2, float sliceAdd = (float)Math.PI / 4f)
	{
		...
		{
			Vector2 vector = new Vector2((float)Math.Round(Math.Sin(sineCounter + sliceAdd * (float)num) * (double)amplitude), sliceSize * num);
			SpriteBatch.Draw(tex.Texture.Texture, position, clipRect, color, rotation, origin - vector, scale, effects, 0f);
			num++;
	...
	public static void TextureBannerV(MTexture tex, Vector2 position, Vector2 origin, Vector2 scale, float rotation, Color color, SpriteEffects effects, float sineCounter, float amplitude = 2f, int sliceSize = 2, float sliceAdd = (float)Math.PI / 4f)
	{
		...
		while (clipRect.Y < tex.ClipRect.Y + tex.ClipRect.Height)
		{
			...
			clipRect.Height = (int)MathHelper.Lerp(sliceSize, 1f, num2);
			clipRect.Height = Math.Min(sliceSize, tex.ClipRect.Y + tex.ClipRect.Height - clipRect.Y);
			Vector2 vector = new Vector2((float)Math.Round(Math.Sin(sineCounter + sliceAdd * (float)num) * (double)amplitude * (double)num2), clipRect.Y - tex.ClipRect.Y);
			SpriteBatch.Draw(tex.Texture.Texture, position, clipRect, color, rotation, origin - vector, scale, effects, 0f);
			num++;
