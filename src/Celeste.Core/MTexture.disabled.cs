...
public class MTexture
{
	...
	{
		int num = (int)((float)ClipRect.X - DrawOffset.X + (float)x);
		int num2 = (int)((float)ClipRect.Y - DrawOffset.Y + (float)y);
		int num3 = (int)MathHelper.Clamp(num, ClipRect.Left, ClipRect.Right);
		int num4 = (int)MathHelper.Clamp(num2, ClipRect.Top, ClipRect.Bottom);
		int width2 = Math.Max(0, Math.Min(num + width, ClipRect.Right) - num3);
		int height2 = Math.Max(0, Math.Min(num2 + height, ClipRect.Bottom) - num4);
