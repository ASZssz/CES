...
public class ColliderList : Collider
{
	...
	{
		Collider[] array = new Collider[colliders.Length + toAdd.Length];
		...
		{
			array[j + colliders.Length] = toAdd[j];
	...
	{
		Collider[] array = new Collider[colliders.Length - toRemove.Length];
		int num = 0;
	...
	{
		Collider[] array = new Collider[colliders.Length];
		for (int i = 0; i < colliders.Length; i++)
		{
			array[i] = colliders[i].Clone();
		}
