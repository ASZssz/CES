...
public class Scene : IEnumerable<Entity>, IEnumerable
{
	...
	public Vector2 LineWalkCheck(Vector2 from, Vector2 to, int tag, float precision)
	{
		...
		vector.Normalize();
		vector *= precision;
		int num = (int)Math.Floor((from - to).Length() / precision);
		Vector2 result = from;
		Vector2 vector2 = from + vector;
		for (int i = 0; i <= num; i++)
		{
			if (CollideCheck(vector2, tag))
			{
				return result;
			}
			result = vector2;
	...
	public Vector2 LineWalkCheckByComponent<T>(Vector2 from, Vector2 to, float precision) where T : Component
	{
		...
		for (int i = 0; i <= num; i++)
		{
			if (CollideCheckByComponent<T>(vector2))
			{
				return result;
			}
			result = vector2;
	...
	{
		double value = 0.0;
		if (actualDepthLookup.TryGetValue(entity.depth, out value))
		{
		...
		entity.actualDepth = (double)entity.depth - value;
